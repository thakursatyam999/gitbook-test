...
You can perform Continuous Integration (CI) in Harness using the CI module and [CI pipelines](../../../../delivery/continuous-integration/new-to-harness-ci/key-concepts.md). If you use Harness Continuous Delivery (CD) but not Harness CI, you can still perform CI using the **Jenkins** step in a CD stage.

You connect Harness to Jenkins using a Harness Jenkins connector. This connector allows you to run Jenkins jobs in [Jenkins steps](../../../../delivery/continuous-delivery/use-continuous-delivery/cd-building-blocks/cd-steps/builds/run-jenkins-jobs-in-cd-pipelines.md).
...
Harness supports SAML authentication for Jenkins connections.
{% endhint %}
...
Add a Jenkins connector in Harness
{% endembed %}
...
5. Enter the URL of the Jenkins master or controller:
...
6. In **Authentication**, select one of the following options to authenticate with the Jenkins server:
...
connector:
  ...
  projectIdentifier: yourprojecttidentifier
  type: Jenkins
  spec:
    jenkinsUrl: yourjenkinsurl
    auth:
...
- [Run Jenkins jobs in CD pipelines](../../../../delivery/continuous-delivery/use-continuous-delivery/cd-building-blocks/cd-steps/builds/run-jenkins-jobs-in-cd-pipelines.md): Configure a Jenkins step to run Jenkins jobs in your pipeline.
